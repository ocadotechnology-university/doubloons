...
trait DoubloonRepository {
  ...
  def createDoubloon(
      doubloon: Doubloon
  ): IO[Either[DoubloonRepository.Failure, Int]]
  def updateDoubloon(
      doubloon: Doubloon
  ): IO[Either[DoubloonRepository.Failure, Int]]
  def deleteDoubloon(
  ...
  def getDoubloonsSpentByOthers(
      email: String,
      monthAndYear: String
  ): IO[List[SpentByOthers]]

  def getDoubloonsSummary(
      givenTo: String,
      monthAndYear: String
  ): IO[List[DoubloonSummary]]
...
object DoubloonRepository {
  ...
  def instance(dbConfig: DatabaseConfig): DoubloonRepository =
    new DoubloonRepository {
      ...
      override def createDoubloon(
        ...
        sql"""INSERT INTO doubloons (category_id, given_to, given_by, amount, month_and_year)
           VALUES (${doubloon.categoryId}, ${doubloon.givenTo},
            ${doubloon.givenBy}, ${doubloon.amount}, ${doubloon.monthAndYear}) RETURNING doubloon_id""".update
      ...
      override def updateDoubloon(
        ...
        sql"""UPDATE doubloons
            ...
            WHERE doubloon_id = ${doubloon.doubloonId}""".update.run
          .transact(dbConfig.xa)
          .attemptSql
